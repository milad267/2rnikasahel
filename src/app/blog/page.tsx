...
import { MobileBlogCarousel } from "@/components/blog/MobileBlogCarousel";
    ...
    <div className="min-h-screen px-4 pb-20 pt-40 sm:px-6 lg:px-8 lg:pt-44">
      <div className="mx-auto max-w-[96rem]">
        ...
        {posts.length === 0 ? (
          ...
          <>
            {/* موبایل: کاروسل با درگ */}
            <MobileBlogCarousel posts={posts} />
            {/* دسکتاپ: گرید ۳ ستونه */}
            <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {posts.map(post => (
                <Link key={post.id} href={`/blog/${post.slug}`} className="card group overflow-hidden rounded-[1.75rem] transition-all hover:shadow-lg" draggable={false}>
                  <div className="aspect-video bg-gradient-to-br from-navy-900/5 to-petrol-100">
                    {post.featuredImage ? <img src={post.featuredImage} alt={post.title} className="size-full object-cover" draggable={false} /> : <div className="flex h-full items-center justify-center text-navy-700/30 text-lg font-bold">📄</div>}
                  </div>
                  <div className="p-5">
