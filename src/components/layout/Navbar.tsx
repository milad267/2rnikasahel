        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div
            className={cn(
              ...
              scrolled ? "shadow-[var(--shadow-luxe)]" : "shadow-[0_16px_50px_-40px_rgba(8,25,43,0.6)]",
            )}
          >
            {/* ردیف بالا: فقط لوگو بالاتر از بقیه */}
            <div className="flex items-center justify-between gap-3">
              <Link href="/" className="shrink-0 min-w-0" aria-label="درنیکا ساحل">
                <Logo className="[&_[class*='flex-col']]:min-w-0" />
              </Link>

              {/* موبایل: دکمه‌ها کنار لوگو */}
            ...
            <div className="mt-2 hidden items-center justify-between gap-3 lg:flex">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={() => setPopup("wishlist")}
                  className="relative flex size-9 items-center justify-center rounded-full bg-petrol-600 text-pearl-50 shadow-[var(--shadow-glow-petrol)] transition-all hover:bg-petrol-500 hover:scale-105"
                >
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] lg:hidden">
            ...
            >
              <div className="flex items-center justify-between">
                <Logo />
                <button type="button" onClick={() => setOpen(false)} className="flex size-10 items-center justify-center rounded-full bg-pearl-100/8 text-pearl-100">
                  ...
                  <motion.div key={l.href} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.05 + i * 0.05 }}>
                    <Link
                      ...
                      className={cn(
                        ...
                        isActive(l.href) ? "bg-petrol-600/25 text-pearl-50" : "bg-pearl-100/[0.04] text-pearl-100 hover:bg-petrol-600/20",
                      )}
                    >
                      {l.label}
                    </Link>
...
function InlineSearch({
  ...
  return (
    <div className={cn("relative", mobile ? "block" : "hidden md:block")} onBlur={() => setTimeout(() => setFocused(false), 150)}>
      <form
        onSubmit={(e) => {
          e.preventDefault();
          if (query.trim()) {
      ...
      >
        <div
          className={cn(
            "flex h-9 items-center gap-2 rounded-full border px-3 transition-all duration-300",
            mobile
              ? "w-full border-pearl-100/15 bg-pearl-100/8"
              : focused
      ...
      <AnimatePresence>
        {focused && (results.length > 0 || searching || query.trim().length >= 2) && (
          <motion.div
            ...
            className={cn("card absolute z-50 overflow-hidden rounded-2xl p-2 shadow-2xl", mobile ? "inset-x-0 top-12" : "end-0 top-12 w-80")}
          >
